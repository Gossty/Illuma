...
function TopArtistSong(props) {
  ...
  return (
    <div className='top-artist-song flex mx-3 font-light hover:cursor-pointer hover:text-violet-500 rounded-xl' onClick={() => toTrack(props.trackId)}>
      ...
      <div className='flex py-[0.375rem] w-[320px]'>
        ...
        <div>
          <p className='whitespace-nowrap w-[130px] overflow-x-auto scrollbar-hide'
          onClick={() => {
            props.windowDimensions.width <= 640 ? toAlbum(props.albumId)
              : ""
            }}
          >{props.song_name}</p>
      ...
      {props.windowDimensions.width > 640 ?
        <div className='flex pb-3 w-[200px] py-[0.375rem] ml-10 text-white'>
          <p className='whitespace-nowrap w-[130px] overflow-x-auto scrollbar-hide hover:underline hover:cursor-pointer hover:text-spotify-400' onClick={() => toAlbum(props.albumId)}>{props.albumName}</p>
        </div>
        :
        <div className='w-[100px] text-gray-500 py-[0.375rem]'>
